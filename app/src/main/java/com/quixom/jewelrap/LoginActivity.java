...
public class LoginActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedPreferences = getSharedPreferences("Paid",MODE_PRIVATE);
...
//                if (!userid.matches(MobilePattern))
//                {
//                    et_username.setError("Please enter valid number");
...
//                }
    ...
    private void populateData(String response) {
        try {
            ...
            if (str.equals("ok")) {
                ...
                session.createLoginSession(name, token, id, userrole, uniqueid, is_verified, firm_name, firstname, lastname , userid , pwd , deviceId);
    ...
    loginUser() {
        ...
        Call<loginBean> call = cr.login(userid , pwd , deviceId);
        ...
        call.enqueue(new Callback<loginBean>() {
            ...
            public void onResponse(Call<loginBean> call, Response<loginBean> response) {
                ...
                Log.e("qqq2", " qwerty1" +response.body().getStatus());

                //msg  = ;
                if (str.equals("ok")) {
                    String name = response.body().getUsername();
                    Log.e("qqq2", " qwerty1" +response.body().getUsername());
                    String token = response.body().getKey();
                    Log.e("JEWELRAP", " token ::" + response.body().toString());
                    ...
                    Log.e("qqq1", " qwerty2" +String.valueOf(response.body().getIsVerified()));
                    String is_paid = String.valueOf(response.body().getIsPaid());
                    String firm_name = response.body().getFirmName();
                    String firstname = response.body().getFirstName();
                    String lastname = response.body().getLastName();
                    Log.e("qqq", " qwerty3" +String.valueOf(response.body().getIsPaid()));
                    session.createLoginSession(name, token, id, userrole, uniqueid, is_verified, firm_name, firstname, lastname , userid ,pwd , deviceId);

                    if (is_verified.equals("false")) {
                        regId = "";
                        android.support.v7.app.AlertDialog.Builder dlgAlert = new android.support.v7.app.AlertDialog.Builder(LoginActivity.this, R.style.DesignDemo);
                        dlgAlert.setMessage("user not verified");
                        dlgAlert.setPositiveButton("OK", null);
                        dlgAlert.setCancelable(true);
                        dlgAlert.create().show();
